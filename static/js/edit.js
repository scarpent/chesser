...
export function editApp() {
  return {
    ...
    goToStartingPosition() {
      this.status = "🟣🟣";
      this.quizMoveIndex = this.editData.start;
      if (this.editData.start >= 0) {
        for (let i = 0; i <= this.editData.start; i++) {
          const quizMove = this.editData.moves[i];
    ...
    //--------------------------------------------------------------------------------
    noMoreMoves() {
      return this.quizMoveIndex >= this.editData.moves.length;
    },
    ...
    playOpposingMove() {
      setTimeout(() => {
        ...
        const san = this.editData.moves[this.quizMoveIndex].san;
    ...
    checkQuizMove(move) {
      ...
      const answer = this.editData.moves[this.quizMoveIndex];
    ...
    //--------------------------------------------------------------------------------
    reportResult(passed) {
      const variationId = this.editData.variation_id;
